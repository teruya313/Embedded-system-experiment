...
public class Kadai9_DrawHeart
{
    ...
    public void drawFigure() {
        ...
        bezier.addPoint( offsetX - 0 , offsetY - 40 ); //始点の座標 (1)
        bezier.addPoint( offsetX - 80 , offsetY + 20 ); //制御線の座標 (2)
        bezier.addPoint( offsetX - 20 , offsetY + 60 ); //制御線の座標 (3)
        bezier.addPoint( offsetX - 0 , offsetY + 20 ); //終点の座標 (4)
        drawPoints.addAll( bezier.getPoints() ); //補間した座標列を取得し描画データに追加

        bezier = new Bezier(); //2本目の曲線を生成する
        bezier.addPoint( offsetX + 0 , offsetY + 20 ); //始点の座標 (5)
        bezier.addPoint( offsetX + 20 , offsetY + 60 ); //制御線の座標 (6)
        bezier.addPoint( offsetX + 80 , offsetY + 20 ); //制御線の座標 )
        bezier.addPoint( offsetX + 0 , offsetY - 40 ); //終点の座標 (8)
    ...
    private void systemExit() {
        hardware.reset();
        System.out.println( "Kadai 9 End." );
        System.exit( 0 );
    }
}
